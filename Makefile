...
graph:
	...
	@cp -f cmd/cyy/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=cyy cyy
	@rm -f main.go cyy.gv
...
.PHONY: build
# build cyy for linux amd64 binary
build:
	@echo "building 'cyy', linux binary file will output to 'cmd/cyy'"
	@cd cmd/cyy && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
